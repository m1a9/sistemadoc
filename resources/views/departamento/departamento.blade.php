<div class="card card-primary">
   ...
   <div class="card-body" style="border: 1px solid #3c8dbc;">
	   <div class="form-group">
			 <button type="button" class="btn" style="background-color: #3c8dbc; color: white;" @click.prevent="nuevo()"><i class="fa fa-plus-square-o" aria-hidden="true" ></i> Nuevo Departamento</button>
	   </div>
   </div>
</div>
...
<div class="card" v-if="diveditar" style="border: 1px solid #00a65a;">
   ...
   <div class="card-body" style="border: 1px solid #00a65a;">
	   <div class="form-group">
		<label>Pais</label>
		<select name="cboPaises" id="cboPaises" v-model="cbopaise" class="form-control"  >
			<option value="">Selecciona</option>
			<option v-for="ps, key in pais" v-bind:value="ps.id">@{{ps.name}}</option>
		</select>
		   <label>Departamento</label>
		   <input type="text" class="form-control" id="txtdepa" v-model="newDepa" autocomplete="off">
	   </div>
   ...
   <div class="card-body" style="border: 1px solid #00a65a;">
	<div class="form-group">
		...
		<select name="cboPaises" id="cboPaises" class="form-control">
		   ...
		   <label>Departamento</label>
		   <input type="text" class="form-control" id="txtdepa" name="name" v-model="newDepa" autocomplete="off">
	   </div>
	   <div class="card-footer bg-transparent">
		   ...
		   <button class="btn btn-primary" v-on:click="probar()">probar</button>
	   </div>
   </div>
</div>
{{-- </form> --}}
<div class="card" style="border: 1px solid #3c8dbc;">
   <div class="card-header" style="border: 1px solid #3c8dbc;background-color: #3c8dbc; color: white;">
	   ...
	   <div class="card-tools">
		   <input type="text" class="form-control form-control-sm" v-model="buscar" v-on:keyup="buscardepartamento" placeholder="Buscar...">
	   </div>
   </div>
	   ...
	   <table class="table table-hover table-bordered table-striped">
		   <thead>
			   <tr>
				   ...
				   <th style="border:1px solid #ddd;padding: 5px; width: 35%;">Departamento</th>
				   <th style="border:1px solid #ddd;padding: 5px; width: 35%;">Pais</th>
				   <th style="border:1px solid #ddd;padding: 5px; width: 20%;">Gestión</th>
				   
			   </tr>
		   </thead>
		   <tbody>
			   <tr v-for="cat, key in departamentos">
				   ...
				   <td style="border:1px solid #ddd;font-size: 14px; padding: 5px;">@{{cat.paisedepa}}</td>
				   <td style="border:1px solid #ddd;font-size: 14px; padding: 5px;">
					   <center>
					   <a href="#" class="btn btn-warning btn-sm" v-on:click.prevent="editar(cat)" data-placement="top" data-toggle="tooltip" title="Editar Departamento">
