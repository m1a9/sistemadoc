...
<div class="card" v-if="diveditar" style="border: 1px solid #00a65a;">
   ...
   <div class="card-body" style="border: 1px solid #00a65a;">
	<div class="form-group">
		<label>Pais</label>
		<select name="cboPaises" id="cboPaises" v-model="cboPaises" class="form-control">
			<option value="">Selecciona</option>
			<option v-for="ps, key in pais" v-bind:value="ps.id">@{{ ps.name }}</option>
		</select>

		<label>Departamento</label>
		<select name="cboDepartamentos" id="cboDepartamentos" v-model="cboDepartamentos" class="form-control">
			<option value="">Selecciona</option>
			<option v-for="ps, key in depa" v-bind:value="ps.id">@{{ ps.name }}</option>
		</select>

		<label>Provincia</label>
		<input type="text" class="form-control" id="txtprov" name="name" v-model="newProvi">
	</div>
	   ...
	   <table class="table table-hover table-bordered table-striped">
		   <thead>
			   <tr>
				   ...
				   <th style="border:1px solid #ddd;padding: 5px; width: 25%;">Provincias</th>
				   <th style="border:1px solid #ddd;padding: 5px; width: 25%;">Departamento</th>
				   <th style="border:1px solid #ddd;padding: 5px; width: 20%;">Pais</th>
				   <th style="border:1px solid #ddd;padding: 5px; width: 20%;">Gestión</th>
				   
			   </tr>
		   </thead>
		   <tbody>
			   <tr v-for="cat, key in provincias">
				   ...
				   <td style="border:1px solid #ddd;font-size: 14px; padding: 5px;">
					   <center>
					   <a href="#" onclick="onComboSelect()" class="btn btn-warning btn-sm" v-on:click.prevent="editar(cat)" data-placement="top" data-toggle="tooltip" title="Editar Departamento">
